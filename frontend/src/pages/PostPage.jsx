...
export const PostPage = () => {
  ...
  const [recentPosts, setRescenPosts] = useState(null);
  ...
  useEffect(() => {
    ...
    const fetchpost = async () => {
      try {
        setLoading(true);
        const res = await fetch(`/api/v1/get-post?slug=${postSlug}`);
  ...
  useEffect(() => {
    try {
      const fetchRecentPost = async () => {
        const res = await fetch(`/api/v1/get-post?limit=3`);
        const data = await res.json();
        if (res.ok) {
          setRescenPosts(data.posts);
        }
      };
      fetchRecentPost();
    } catch (error) {
